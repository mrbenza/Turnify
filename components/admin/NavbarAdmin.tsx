...
/* ------------------------------------------------------------------ */
/* Area selector dropdown — for managers with multiple areas           */
/* ------------------------------------------------------------------ */
...
function AreaSelector({ areas, activeAreaId, onSwitch }: AreaSelectorProps) {
  ...
  if (areas.length <= 1) {
    return (
      <div className="mx-3 mb-3 px-3 py-1.5 rounded-lg bg-blue-50 text-xs font-medium text-blue-700 truncate">
        {activeArea?.nome ?? '—'}
      </div>
    )
  }
  ...
  return (
    <div ref={ref} className="mx-3 mb-3 relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-center justify-between px-3 py-1.5 rounded-lg bg-blue-50 text-xs font-medium text-blue-700 hover:bg-blue-100 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-400"
        aria-haspopup="listbox"
        aria-expanded={open}
        aria-label="Seleziona area"
      >
        <span className="truncate">{activeArea?.nome ?? '—'}</span>
        <svg
          className={`w-3.5 h-3.5 ml-1.5 shrink-0 transition-transform ${open ? 'rotate-180' : ''}`}
      ...
      {open && (
        ...
        >
          {areas.map((area) => {
            ...
            return (
              <li key={area.id} role="option" aria-selected={isActive}>
                <button
                  onClick={() => { onSwitch(area.id); setOpen(false) }}
                  className={`w-full text-left px-3 py-2 text-xs font-medium transition-colors focus:outline-none focus:bg-blue-50 ${
...
function SidebarContent({ pathname, nomeAdmin, allNavItems, managedAreas, activeAreaId, onLinkClick, onLogout, onAreaSwitch }: SidebarContentProps) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Area selector — solo per manager con area assegnata */}
      {managedAreas && managedAreas.length > 0 && activeAreaId && onAreaSwitch && (
        <AreaSelector areas={managedAreas} activeAreaId={activeAreaId} onSwitch={onAreaSwitch} />
      )}
...
interface NavbarAdminProps {
  nomeAdmin?: string
  ruolo?: 'admin' | 'manager'
  /** Aree gestite dal manager (inclusa la sua home area). Solo per ruolo manager. */
  managedAreas?: Area[]
  /** ID dell'area attualmente selezionata (da cookie o default). */
  activeAreaId?: string
}

export default function NavbarAdmin({ nomeAdmin, ruolo, managedAreas, activeAreaId }: NavbarAdminProps) {
      ...
      {moreOpen && (
        <>
          ...
          >
            ...
            {/* Area selector mobile — solo manager con più aree */}
            {managedAreas && managedAreas.length > 0 && activeAreaId && (
              <div className="px-5 py-3 border-b border-gray-100">
                <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-400 mb-2">Area attiva</p>
                <div className="flex flex-wrap gap-2">
                  {managedAreas.map((area) => {
                    ...
                    return (
                      <button
                        key={area.id}
                        onClick={() => { handleAreaSwitch(area.id); setMoreOpen(false) }}
