  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <>
          {/* Mobile cards (< sm) */}
          <div className="sm:hidden space-y-3" aria-label={`Lista turni ${MONTH_NAMES[filterMonth]} ${filterYear}`}>
            {shifts.map((shift) => (
              <div
                key={shift.id}
              ...
              >
                ...
                <div className="flex items-start justify-between gap-3 mb-2">
                  <div>
                    <p className="text-base font-bold text-gray-900">{formatDate(shift.date)}</p>
                    <p className="text-xs text-gray-400 mt-0.5">{getDayName(shift.date)}</p>
                  </div>
                  <span className={`shrink-0 inline-block px-2 py-1 rounded-full text-xs font-medium ${SHIFT_TYPE_STYLES[shift.shift_type]}`}>
                    {SHIFT_TYPE_LABELS[shift.shift_type]}
                  </span>
                </div>

                {/* Employee */}
                <p className="text-sm font-medium text-gray-800 mb-3">{shift.userName}</p>

                {/* Remove button */}
                {!locked && (
                  <div className="flex justify-end">
                    {removing === shift.id ? (
                      <span className="text-gray-400 flex items-center gap-1 text-xs"><Spinner />Rimozione...</span>
                    ) : (
                      <button
                        onClick={() => setConfirmRemove(shift.id)}
                        className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg border border-red-200 text-red-600 text-xs font-medium hover:bg-red-50 transition-colors focus:outline-none focus:ring-2 focus:ring-red-300 min-h-[44px]"
            ...
            <table className="w-full text-sm" aria-label={`Lista turni ${MONTH_NAMES[filterMonth]} ${filterYear}`}>
              <thead>
                <tr className="border-b border-gray-100">
                  <th scope="col" className="text-left py-2.5 px-0 font-semibold text-gray-500 text-xs uppercase tracking-wide">Data</th>
                  <th scope="col" className="text-left py-2.5 px-2 font-semibold text-gray-500 text-xs uppercase tracking-wide">Giorno</th>
                  <th scope="col" className="text-left py-2.5 px-2 font-semibold text-gray-500 text-xs uppercase tracking-wide">Dipendente</th>
                  <th scope="col" className="text-left py-2.5 px-2 font-semibold text-gray-500 text-xs uppercase tracking-wide">Tipo</th>
                  <th scope="col" className="text-left py-2.5 px-2 font-semibold text-gray-500 text-xs uppercase tracking-wide">Assegnato da</th>
                  {!locked && (
                    <th scope="col" className="py-2.5 px-2 text-xs uppercase tracking-wide">
              ...
              <tbody className="divide-y divide-gray-50">
                {shifts.map((shift) => (
                  <tr key={shift.id} className="hover:bg-gray-50 transition-colors">
                    <td className="py-2.5 px-0 text-gray-700 whitespace-nowrap">{formatDate(shift.date)}</td>
                    <td className="py-2.5 px-2 text-gray-500 whitespace-nowrap">{getDayName(shift.date)}</td>
                    <td className="py-2.5 px-2 font-medium text-gray-800">{shift.userName}</td>
                    <td className="py-2.5 px-2">
                      <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${SHIFT_TYPE_STYLES[shift.shift_type]}`}>
                        {SHIFT_TYPE_LABELS[shift.shift_type]}
                      </span>
                    </td>
                    <td className="py-2.5 px-2 text-gray-500">{shift.createdByName}</td>
                    {!locked && (
